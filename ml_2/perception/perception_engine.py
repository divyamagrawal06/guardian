...
class PerceptionEngine:
    ...
    def perceive(self, monitor: Optional[int] = None, use_vlm: bool = True) -> PerceptionResult:
        ...
        # Steps 4-5: OCR + VLM in parallel (if VLM enabled)
        vlm_regions = []
        screen_description = ""
        
        if use_vlm:
            # Run OCR and VLM concurrently for ~2x speedup
            with ThreadPoolExecutor(max_workers=2) as pool:
                ocr_future = pool.submit(self.ocr.detect, frame.image)
                vlm_future = pool.submit(self._run_vlm, frame.image)
                
                ocr_raw = ocr_future.result()
                vlm_raw, screen_description = vlm_future.result()
    ...
    def _run_vlm(self, image):
        """Run VLM detection + description (for parallel execution)."""
        vlm_raw = self.vlm.detect_ui_elements(image)
        screen_desc = self.vlm.describe_screen(image)
        return vlm_raw, screen_desc
