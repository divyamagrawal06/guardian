...
class ScreenCapture:
    ...
    def __init__(self, screen_config: Optional[ScreenConfig] = None):
        ...
        detected_scale = self.detect_dpi_scale()
        if detected_scale != 1.0:
            self.config.dpi_scale = detected_scale
            logger.info(f"DPI scale set to {detected_scale}")
        
    def _set_dpi_awareness(self) -> None:
        """Set process DPI awareness on Windows for accurate coordinates."""
        try:
            import ctypes
            # PROCESS_PER_MONITOR_DPI_AWARE = 2
            ctypes.windll.shcore.SetProcessDpiAwareness(2)
            logger.debug("Set DPI awareness to per-monitor")
        except Exception:
            try:
                import ctypes
